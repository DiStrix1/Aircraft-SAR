...
SCENARIO_DESCRIPTIONS: Dict[str, str] = {
    ...
    "ditching": (
        "Controlled Ditching — Pilot attempts a controlled water/surface landing "
        "after engine failure. Aircraft glides toward a landing site with a final "
        "splash scatter of 0.5–3 km radius around the planned touch-down point."
    ),
}
...
DEFAULT_SCENARIO_WEIGHTS: Dict[str, float] = {
    ...
    "dive": 0.20,
    "breakup": 0.10,
    "ditching": 0.00,  # off by default — enabled by user or over-water presets
}
...
def _gaussian_heading(heading_deg: float, spread_deg: float) -> float:
    """Return a heading randomly perturbed using a Gaussian distribution.

    The standard deviation is set to spread_deg / 3 so that ~99.7% of samples
    fall within ±spread_deg of the original heading.  The result is clamped to
    [heading_deg - spread_deg, heading_deg + spread_deg] to avoid outliers.
    """
    if spread_deg <= 0:
        return heading_deg
    sigma = spread_deg / 3.0
    delta = random.gauss(0.0, sigma)
    delta = max(-spread_deg, min(spread_deg, delta))
    return (heading_deg + delta) % 360
...
def monte_carlo_simulation(
    ...
    Uses five descent archetypes (best_glide, spiral, dive, breakup, ditching) with
    multi-layer wind at each altitude band.  Heading perturbation now uses a
    Gaussian distribution (concentrated near original heading, ±spread_deg at 3σ).
    ...
    heading_spread_deg : float
        Maximum random deviation applied to heading (Gaussian, 3-sigma boundary).
    ...
    seed : int | None
        ...
        If None (default), RNG is left unseeded for truly random results.
    """
    ...
    iterations = max(1, min(iterations, 10_000))  # clamp to safe range
    if iterations != original_iterations:
        _log.warning(
            "Iterations clamped from %d to %d (allowed range: 1–10,000).",
            original_iterations, iterations,
        )

    # Seed RNG only when explicitly requested (e.g. preset reproducibility)
    if seed is not None:
        random.seed(seed)

    # Zero altitude → aircraft is already on the ground
    if altitude_ft <= 0:
        return [(last_lat, last_lon)] * iterations
    ...
    if scenario_weights is not None:
        weights = dict(scenario_weights)  # defensive copy
        # Guard against all-zero weights
        total = sum(weights.values())
        if total <= 0:
            weights = dict(DEFAULT_SCENARIO_WEIGHTS)
        # Ensure all archetypes exist in the dict (backwards compat)
        for k in DEFAULT_SCENARIO_WEIGHTS:
    ...
    for _ in range(iterations):
      try:
        ...
        # Gaussian heading spread — concentrated near original heading
        h_var = _gaussian_heading(heading_deg, heading_spread_deg)
        ...
        if scenario == "best_glide":
        ...
        elif scenario == "ditching":
            # ── Controlled Ditching: best-glide toward landing zone ──
            lat, lon = project_glide_position_multilayer(
                lat, lon, h_var,
                remaining_alt, glide_ratio, best_glide_speed,
                w_var, w_dir_var,
            ...
            # Final splash scatter — small radius around the touchdown point
            splash_dist = random.uniform(0.5, 3.0)
            splash_bearing = random.uniform(0, 360)
            lat, lon = destination_point(lat, lon, splash_bearing, splash_dist)
            ...
            # Wind drift through altitude column (lateral + along-track)
            lateral_drift_km, along_drift_km = multi_layer_wind_drift(
            ...
            # Along-track: push along or against the heading
            if along_drift_km >= 0:
                lat, lon = destination_point(lat, lon, h_var, along_drift_km)
            else:
                lat, lon = destination_point(lat, lon, (h_var + 180) % 360, abs(along_drift_km))
            ...
            # Wind drift — lateral + along-track
            lateral_drift_km, along_drift_km = multi_layer_wind_drift(
                w_var, w_dir_var, h_var,
            ...
            # Wind-driven lateral + along-track drift
            lateral_drift_km, along_drift_km = multi_layer_wind_drift(
            ...
            drift_bearing = (h_var + 90) % 360 if lateral_drift_km >= 0 else (h_var - 90) % 360
            lat, lon = destination_point(lat, lon, drift_bearing, abs(lateral_drift_km))
            along_drift_km *= wind_sensitivity
            if along_drift_km >= 0:
                lat, lon = destination_point(lat, lon, h_var, along_drift_km)
            else:
                lat, lon = destination_point(lat, lon, (h_var + 180) % 360, abs(along_drift_km))
        ...
        # Single-iteration failure should not crash the entire simulation.
        # Skip this iteration silently and continue.
        continue

    # If no points were generated (all iterations failed), return LKP
    if not points:
...
# ---------------------------------------------------------------------------
# Probability-zone classification (KDE-based)
# ---------------------------------------------------------------------------
def generate_probability_zones(
    points: List[LatLon],
    center: LatLon,
) -> Dict[str, List[LatLon]]:
    """Classify *points* into HIGH / MEDIUM / LOW probability zones.

    Uses Kernel Density Estimation (KDE) to identify regions of highest
    point concentration.  Points in the densest 33% are HIGH, next 33%
    are MEDIUM, and the remainder LOW.  This reflects actual spatial
    clustering rather than simple distance from centroid.

    Falls back to distance-based percentile classification if KDE fails
    (e.g. all points identical or scipy unavailable).
    """
    if not points:
    ...
    try:
        ...
        # Scale lon by cos(lat) so KDE operates in roughly equal distances
        cos_factor = np.cos(np.radians(center[0]))
        coords = np.vstack([lats, lons * cos_factor])

        kde = gaussian_kde(coords, bw_method="silverman")
        densities = kde(coords)

        # Higher density = higher probability
        p67 = np.percentile(densities, 33)   # bottom 33% density = LOW
        p33 = np.percentile(densities, 67)   # top 33% density = HIGH

        zones: Dict[str, List[LatLon]] = {"HIGH": [], "MEDIUM": [], "LOW": []}
        for pt, d in zip(points, densities):
            if d >= p33:
        ...
        # Fallback: distance-based percentile classification
        distances = [
