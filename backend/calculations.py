...
def haversine_distance(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    ...
    Returns 0.0 for invalid (NaN/Inf) inputs instead of propagating errors.
    """
    try:
        ...
        a = min(max(a, 0.0), 1.0)  # clamp to valid domain for asin/atan2
...
def destination_point(lat: float, lon: float, bearing_deg: float, distance_km: float) -> Tuple[float, float]:
    ...
    Returns the origin point unchanged if distance is zero or negative.
    """
    if distance_km <= 0:
        return lat, lon
    φ1 = math.radians(lat)
...
def calculate_glide_distance(
    ...
    """
    if altitude_ft <= 0 or glide_ratio <= 0:
        return 0.0
    altitude_km = altitude_ft * FEET_TO_KM
    # Symmetric parabolic speed-efficiency: deviation in either direction from
    # best-glide speed reduces the effective glide ratio.  The coefficient k=2.0
    # gives ~10% penalty at 20% speed deviation, matching typical POH data.
    if best_glide_speed_kts and best_glide_speed_kts > 0:
        ratio = airspeed_kts / best_glide_speed_kts
        k = 2.0
        speed_efficiency = max(1.0 - k * (ratio - 1.0) ** 2, 0.1)  # floor at 10%
    else:
...
def calculate_along_track_wind(
    wind_speed_kts: float,
    wind_direction_deg: float,
    heading_deg: float,
    time_minutes: float,
) -> float:
    """Return **along-track** wind displacement in km (parallel to heading).

    Positive = pushes in the heading direction (tailwind component).
    Negative = pushes against the heading (headwind component).
    """
    relative_angle = math.radians(wind_direction_deg - heading_deg)
    along_kts = wind_speed_kts * math.cos(relative_angle)
    along_kmh = along_kts * KNOTS_TO_KMH
    return along_kmh * (time_minutes / 60.0)
...
def project_position(
    ...
    Wind convention: **meteorological** — *wind_direction_deg* is the direction
    the wind is blowing **FROM** (e.g. 270° = westerly wind blowing eastward).
    Internally this is converted to a velocity vector pointing in the direction
    the wind is actually moving, then added to the aircraft TAS vector to get
    the ground-speed track.
...
def multi_layer_wind_drift(
    ...
    """Compute cumulative lateral AND along-track drift through altitude layers.

    Returns (lateral_drift_km, along_track_drift_km).
    ...
    for _ in range(layers):
        ...
        total_lateral += calculate_wind_drift(
            layer_wind_spd, layer_wind_dir, heading_deg, layer_time
        )
        total_along += calculate_along_track_wind(
            layer_wind_spd, layer_wind_dir, heading_deg, layer_time
        )
        current_alt = max(current_alt - descent_rate_fpm * layer_time, 0)

    return total_lateral, total_along
...
def project_glide_position_multilayer(
    ...
    *wind_profile* is provided, real wind data is used; otherwise the
    ...
    if altitude_ft <= 0 or descent_rate_fpm <= 0:
        return lat, lon
    if glide_ratio <= 0 or best_glide_speed_kts <= 0:
        return lat, lon
    layers = max(layers, 1)  # guard against 0 or negative layers
