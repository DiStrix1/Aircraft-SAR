...
class TestNewAircraftSpecs(unittest.TestCase):
    """Verify all newly added aircraft are present and valid."""
    ...
    def test_new_aircraft_exist(self):
        for name in self.NEW_AIRCRAFT:
            self.assertIn(name, AIRCRAFT_DATA, f"{name} missing from AIRCRAFT_DATA")

    def test_new_aircraft_have_required_fields(self):
        for name in self.NEW_AIRCRAFT:
            data = AIRCRAFT_DATA[name]
            self.assertIn("glide_ratio", data, f"{name} missing glide_ratio")
            self.assertIn("cruise_speed", data, f"{name} missing cruise_speed")
            self.assertIn("best_glide_speed", data, f"{name} missing best_glide_speed")
            self.assertGreater(data["glide_ratio"], 0, f"{name} glide_ratio must be > 0")

    def test_helicopter_glide_ratio_lower_than_jets(self):
        """Helicopters should have lower glide ratios than commercial jets."""
        bell = AIRCRAFT_DATA["Bell 412"]["glide_ratio"]
        a320 = AIRCRAFT_DATA["Airbus A320-200"]["glide_ratio"]
        self.assertLess(bell, a320)


class TestGALowAltitude(unittest.TestCase):
    """Monte Carlo with a Cessna 172S at low altitude — centroid should stay close."""

    def test_low_altitude_centroid_nearby(self):
        points = monte_carlo_simulation(
            iterations=200,
            last_lat=34.05,
            last_lon=-118.25,
        ...
        dist = haversine_distance(34.05, -118.25, centroid_lat, centroid_lon)
        # At 2000ft with 5 min, should be within 25 km of LKP
        self.assertLess(dist, 25, f"Centroid too far from LKP: {dist:.1f} km")

    def test_low_altitude_returns_correct_count(self):
        points = monte_carlo_simulation(
            iterations=50,
            last_lat=34.05,
            last_lon=-118.25,
...
class TestDitchingScenario(unittest.TestCase):
    """Ditching scenario should cluster points near LKP with small splash."""

    def test_ditching_points_close_to_lkp(self):
        points = monte_carlo_simulation(
            iterations=200,
            last_lat=12.0,
            last_lon=77.0,
            ...
            scenario_weights={"best_glide": 0.0, "spiral": 0.0, "dive": 0.0, "breakup": 0.0, "ditching": 1.0},
            seed=3,
        )
        self.assertGreater(len(points), 0)
        dists = [haversine_distance(12.0, 77.0, lat, lon) for lat, lon in points]
        dists.sort()
        # 95th percentile should be within 35 km (a Cessna at 3000ft glides ~8km,
        # plus powered phase can add ~15km, plus heading spread and scatter).
        p95 = dists[int(len(dists) * 0.95)]
        self.assertLess(p95, 35, f"95th-percentile ditching point too far: {p95:.1f} km")


class TestWeatherCache(unittest.TestCase):
    """Verify that get_wind_profile() caches results and avoids duplicate HTTP calls."""

    def test_cache_prevents_double_fetch(self):
        from datetime import datetime, timezone
        from weather_data import get_wind_profile, clear_weather_cache, WindProfile, WindLayer

        clear_weather_cache()

        fake_profile = WindProfile(
            layers=[WindLayer(altitude_ft=300, wind_speed_kts=10, wind_direction_deg=270)],
            source="mock",
            valid_time="2024-01-01T00:00Z",
        )

        call_count = {"n": 0}

        def mock_fetch(lat, lon, dt, timeout=15):
            call_count["n"] += 1
            return fake_profile

        with patch("weather_data._fetch_openmeteo", side_effect=mock_fetch):
            dt = datetime(2024, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
            p1 = get_wind_profile(10.0, 20.0, dt)
            p2 = get_wind_profile(10.0, 20.0, dt)  # same key — should hit cache

        self.assertEqual(call_count["n"], 1, "Expected 1 HTTP call but got more (cache missed)")
        self.assertIsNotNone(p1)
        self.assertIsNotNone(p2)
        clear_weather_cache()

    def test_cache_miss_on_different_location(self):
        from datetime import datetime, timezone
        from weather_data import get_wind_profile, clear_weather_cache, WindProfile, WindLayer
        ...
        fake_profile = WindProfile(
            layers=[WindLayer(altitude_ft=300, wind_speed_kts=10, wind_direction_deg=270)],
            source="mock", valid_time="2024-01-01T00:00Z",
        )
        call_count = {"n": 0}
        ...
        with patch("weather_data._fetch_openmeteo", side_effect=mock_fetch):
            dt = datetime(2024, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
            get_wind_profile(10.0, 20.0, dt)
            get_wind_profile(50.0, 10.0, dt)  # different location — should miss cache

        self.assertEqual(call_count["n"], 2)
        clear_weather_cache()


class TestSearchPatternAPI(unittest.TestCase):
    """Test the /api/search-pattern FastAPI endpoint."""

    def test_search_pattern_returns_waypoints(self):
        try:
        ...
        resp = client.get(
            "/api/search-pattern",
            params={"centroid_lat": 2.98, "centroid_lon": -30.59, "area_km2": 500, "heading": 0},
        )
    ...
    def test_search_pattern_invalid_area(self):
        try:
            from fastapi.testclient import TestClient
            from server import app
        except ImportError:
        ...
        resp = client.get(
            "/api/search-pattern",
            params={"centroid_lat": 2.98, "centroid_lon": -30.59, "area_km2": -1},
        )
        self.assertEqual(resp.status_code, 422)  # validation error


class TestWeatherAPIEndpoint(unittest.TestCase):
    """Test the /api/weather FastAPI endpoint (mocked HTTP)."""

    def test_weather_endpoint_returns_layers(self):
        try:
            from fastapi.testclient import TestClient
            from server import app
        except ImportError:
            self.skipTest("httpx not installed for TestClient")
            return
        from weather_data import clear_weather_cache, WindProfile, WindLayer
        clear_weather_cache()
        fake = WindProfile(
            layers=[WindLayer(altitude_ft=300, wind_speed_kts=15, wind_direction_deg=270)],
            source="mock", valid_time="2024-01-01T00:00Z",
        )
        with patch("weather_data._fetch_openmeteo", return_value=fake):
            client = TestClient(app)
            resp = client.get("/api/weather", params={"lat": 10.0, "lon": 20.0})
        self.assertEqual(resp.status_code, 200)
        data = resp.json()
