...
Provides:
  ...
  • get_wind_profile()   — fetch wind profile from NOAA GFS via NOMADS
  • wind_at_altitude()   — interpolate (speed, direction) at any altitude
  • AF447_WIND_PROFILE   — hardcoded fixture for validation

Data source
-----------
NOAA GFS 0.25° via NOMADS grib-filter.  We request U/V wind components at
eight standard pressure levels and convert to speed + direction.
...
# ---------------------------------------------------------------------------
# Primary data source: returns wind U/V at 8 pressure levels as JSON.
# Works for forecast (±16 days) and historical archive (1940–present).
# No API key, no GRIB parsing required.
...
def get_wind_profile(
    ...
    Tries Open-Meteo first (free JSON API, supports forecast + historical).
    ...
    logger.warning("All weather data sources failed")
    return None
